...
- **(없음)**: **스펙 변경 이력의 최신 게이트 규칙을 우선 적용**(v1.4: G-12 자동 상환은 #9까지 → 이후 G-02 Inc 3 → U-08 → G-01 Inc 2 순. #13/#14는 수동 검증 대기 — 자동 선택 금지)하고, 최신 `GAP_ANALYSIS_V*`의 권장 실행 순서로 완료 기록이 없는 첫 항목을 선택. 선택 근거를 한 줄로 보고 후 진행.
