---
description: CapCut 능가 스펙(G-ID) 작업 착수 — 인자 없으면 다음 우선순위 자동 선택
argument-hint: "[G-ID | debt | status] 예: /surpass G-02, /surpass debt, /surpass"
---

# CapCut 능가 작업 세션

MovieCut의 CapCut 능가 계획을 G-ID 단위로 실행한다. **스펙이 사실의 원천이다** — 아래 순서를 반드시 따른다.

인자: `$ARGUMENTS`

## 0. 컨텍스트 로드 (필수, 순서대로)

1. `docs/CAPCUT_SURPASS_SPEC_20260703.md` — **작업 명세(데이터 모델·구현 증분·AC·검증 계획)**. 이 문서의 G-ID가 작업 단위다.
2. `docs/CAPCUT_GAP_IMPROVEMENT_PLAN_20260703.md` — 격차 분석·우선순위 배경 (§5 착수 순서).
3. `docs/CAPCUT_FEATURE_BACKLOG.md` §2.5 — 증거 기반 검증 현황 (자가보고 수치 신뢰 금지).

## 1. 인자 처리

- **`G-XX`** (예: `G-02`): 해당 항목 착수. 스펙의 선행 의존(마일스톤 순서 원칙)이 미충족이면 먼저 보고하고 사용자 확인.
- **`debt`**: G-12 검증 부채 원장에서 미상환 항목 상위 1~2개를 골라 상환.
- **`status`**: 작업 없이 현황만 보고 — 스펙 AC의 검증 기록 줄과 git log를 대조해 G-ID별 진행 상태 표 출력 후 종료.
- **(없음)**: 스펙 §2 마일스톤 순서(S0→S1→…)에서 완료 기록이 없는 첫 항목을 선택하고, 선택 근거를 한 줄로 보고 후 진행.
