...
final class MaskCompositor {
    static func apply(mask: Mask, to image: CIImage, at time: TimeInterval) -> CIImage {
        ...
        var maskImage = makeMaskImage(for: mask, in: extent)
        if mask.feather > 0 {
            maskImage = maskImage
                .applyingFilter("CIGaussianBlur", parameters: [kCIInputRadiusKey: mask.feather * 10])
                .cropped(to: extent)
        }
    ...
    private static func makeMaskImage(for mask: Mask, in extent: CGRect) -> CIImage {
        ...
        case .triangle:
            return pathMask(points: trianglePoints(for: mask), mask: mask, in: extent)
        case .diamond:
            return pathMask(points: diamondPoints(for: mask), mask: mask, in: extent)
        case .linear:
    ...
    private static func rectangleMask(for mask: Mask, in extent: CGRect) -> CIImage {
        ...
        if mask.rotation == 0 {
            return white.composited(over: clear).cropped(to: extent)
        }
    ...
    private static func ellipseMask(for mask: Mask, in extent: CGRect) -> CIImage {
        ...
        )?.outputImage ?? CIImage(color: CIColor.black)
    ...
    private static func pathMask(points: [CGPoint], mask: Mask, in extent: CGRect) -> CIImage {
