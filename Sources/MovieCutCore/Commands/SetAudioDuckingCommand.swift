...
public struct SetAudioDuckingCommand: EditorCommand {
    ...
    public func apply(to project: inout Project) throws -> CommandResult {
        ...
        return CommandResult(
            ...
            description: "Set audio ducking on \(duckingRangesByClip.count) clips",
            undoValues: ["previousClips": .clips(previousClips)]
        )
    }

    public func invert(from result: CommandResult) throws -> any EditorCommand {
        if case .clips(let previousClips)? = result.undoValues["previousClips"] {
            return RestoreClipDuckingCommand(clips: previousClips)
        }
        return NoOpCommand(description: "Missing previous ducking state for inverse")
    }
}
...
public struct RestoreClipDuckingCommand: EditorCommand {
    ...
    public func apply(to project: inout Project) throws -> CommandResult {
        ...
        for snapshot in clips {
            ...
            affectedClipIds.insert(snapshot.id)
        }

        return CommandResult(
            affectedClipIds: affectedClipIds,
            description: "Restored audio ducking on \(affectedClipIds.count) clips",
            undoValues: [:]
        )
    }

    public func invert(from result: CommandResult) throws -> any EditorCommand {
        NoOpCommand(description: "Ducking restore is not re-invertible")
    }
}
