...
/// Wiring visibility for the Mac/iOS compositor paths (not a completion
/// criterion by itself — see spec DoD §1.3).
@Suite("Canvas Background Static Contract")
struct CanvasBackgroundStaticContractTests {
    private func source(_ path: String) throws -> String {
        try String(contentsOfFile: path, encoding: .utf8)
    }

    @Test("mac compositor composes frames over the shared background processor")
    func macCompositorComposesBackground() throws {
        let compositor = try source("App/MovieCutMac/Export/CustomVideoCompositor.swift")
        #expect(compositor.contains("let canvasBackground: CanvasBackground?"))
        #expect(compositor.contains("CanvasBackgroundPixelProcessor.compose"))
    }

    @Test("mac engines pass the project background into instructions")
    func macEnginesPassBackground() throws {
        let export = try source("App/MovieCutMac/Export/ExportEngine.swift")
        #expect(export.contains("canvasBackground: project.canvasBackground"))

        let playback = try source("App/MovieCutMac/Playback/PlaybackEngine.swift")
        #expect(playback.contains("canvasBackground: project.canvasBackground"))
    }

    @Test("ios compositor and export mirror the background wiring")
    func iosMirrorsBackground() throws {
        let compositor = try source("App/MovieCutiOS/Export/IOSCustomVideoCompositor.swift")
        #expect(compositor.contains("CanvasBackgroundPixelProcessor.compose"))

        let export = try source("App/MovieCutiOS/Export/IOSExportEngine.swift")
        #expect(export.contains("canvasBackground: project.canvasBackground"))
    }

    @Test("mac settings UI exposes background controls")
    func settingsUIExposesBackground() throws {
        let view = try source("App/MovieCutMac/ProjectSettings/CanvasSettingsView.swift")
        #expect(view.contains("onBackgroundChange"))
        #expect(view.contains(".sourceBlur"))
        #expect(view.contains("chooseBackgroundImage"))

        let viewModel = try source("App/MovieCutMac/EditorViewModel.swift")
        #expect(viewModel.contains("func updateCanvasBackground"))
        #expect(viewModel.contains("SetCanvasBackgroundCommand"))
    }
}
