...
struct MediaFixtureTests {
    ...
    @Test("occluded moving subject video loads with the expected tracking fixture properties")
    func movingSubjectOccludedVideoProperties() async throws {
        let asset = AVURLAsset(url: MediaFixtures.movingSubjectOccludedVideo)
        let duration = try await asset.load(.duration)
        #expect(abs(CMTimeGetSeconds(duration) - 3.0) < 0.05)
