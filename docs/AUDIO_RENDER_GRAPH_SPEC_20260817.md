...
**과도기 기준(v1.1)**: Inc 9 게이트는 그래프가 제품 경로가 되기 전이므로 대조 기준을 프리뷰 믹스 렌더로 대체 사용한다(길이 ±0.5s·LUFS/RMS 관대 임계 — AAC 패딩·램프 모양 차이 흡수). 제품 경로 전환 증분에서 그래프 PCM 기준으로 교체하고 ±1 샘플 엄격 게이트로 전환한다.
