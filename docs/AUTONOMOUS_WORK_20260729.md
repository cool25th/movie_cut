...
- [x] **A4** `App/MovieCutMac/Effects/ZoomTransitionPlugin.swift` (83줄, internal). 커밋 `refactor(moviecut): remove dead ZoomTransitionPlugin`. ✅ DONE — GATE_PASS, swift test 984 유지.
...
### ENTRY 4 — 2026-07-29 (cron 발화) — A4 ✅ DONE
- **항목**: A4 — `App/MovieCutMac/Effects/ZoomTransitionPlugin.swift` 제거
- **상태**: DONE
- **재검증**: grep `ZoomTransitionPlugin` → 자기 선언 1건뿐 (83줄, 단일 internal class, TransitionPlugin 준수하나 등록/인스턴스화 0회)
- **게이트 결과**: GATE_PASS — swift build OK / swift test **984/162 suites passed** / xcodebuild MovieCutMac OK
- **변경**: 파일 -83줄 + pbxproj 정리(net -5)
