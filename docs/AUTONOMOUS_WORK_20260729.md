...
- [x] **C2** `docs/GAP_ANALYSIS_V2.md` — `ContentView.swift` → `iOSContentView.swift` 3곳(:14,:189,:208). 커밋 `docs: fix iOSContentView path in gap analysis`. ✅ DONE — GATE_PASS, swift test 984 유지. 큐 명시 3곳 수정. 추가 bare ContentView 2곳(:192,:222)은 산문/iOS문맥 명확 → 보수정책상 그대로 유지, LOG에 기록.
...
### ENTRY 12 — 2026-07-29 (cron 발화) — C2 ✅ DONE
- **항목**: C2 — `docs/GAP_ANALYSIS_V2.md` 의 iOS ContentView 경로 오타 정리
- **상태**: DONE
- **재검증**: 실제 iOS 파일은 `App/MovieCutiOS/iOSContentView.swift` (소문자 i). Mac의 `App/MovieCutMac/ContentView.swift`는 별개. GAP_ANALYSIS_V2.md에서 전체 경로 `App/MovieCutiOS/ContentView.swift`는 2곳(:189,:208) + 산문 bare `ContentView` (:14,:192,:222) 총 5곳.
- **변경**: 큐 명시 3곳 수정 — :14 `ContentView`→`iOSContentView`, :189/:208 전체경로 `ContentView.swift`→`iOSContentView.swift`. 수정 후 전체경로 `App/MovieCutiOS/ContentView.swift` 0건 확인.
- **유지(보수정책)**: 추가 bare `ContentView` 2곳(:192,:222)은 iOS 문맥의 산문 언급이라 판단 없이 그대로 둠. (귀환 후 검토 가능)
- **게이트 결과**: GATE_PASS — swift build OK / swift test **984/162 suites passed** / xcodebuild MovieCutMac OK. (문서만 변경.)

### ENTRY 11 — 2026-07-29 (cron 발화) — C1 ✅ DONE (별도 브랜치, main 미병합)
- **항목**: C1 — PLATFORM_PARITY_MATRIX.md:223 IOSPlaybackEngine 항목 갱신. 브랜치 `docs/mark-iosplaybackengine-removed-parity`, 커밋 `789fc04`.
