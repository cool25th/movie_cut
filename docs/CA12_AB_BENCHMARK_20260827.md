...
1. **HDR 태그 소스의 preview↔export 픽셀 발산(ab03)**: PSNR 15.1dB·MAD p95 15.8. 기존
   ...
   CA-04는 출력 태그(ffprobe)만 확인했고 픽셀 파리티는 미측정이었다. → 결함 후보 등록(§8).
   **심화(같은 날)**: 대부분 밴드 Δ≤2·고채도 시안 밴드만 프리뷰에서 핑크 — 플레이어 다리의
   AVFoundation HDR→SDR 변환이 범위 밖 색을 뒤집음(출력은 원시 재해석으로 소스 프레임과
   MAD 2.49 충실). 시도된 수정 2건(스냅샷 작업공간 핀·합성 709 삼중항) 모두 측정 무효로 폐지 —
   본수정은 G-29 HDR 파이프라인으로 이관(§8).
...
4. **덕킹×파리티 경로 조합 파킹(ab09 — 결정론 재현)**: 파리티 하니스에서 DUCKING 게이트
   ...
   `scenarios_applied`에서 영구 정지). **심화(같은 날)**: 첫 필수 서스펜션 지점마다 파킹이
   이동(순서 재배열로 지점은 옮겨지나 동일 파킹)·`DispatchQueue.main.async`도 전달 정지
   (GCD 레벨)·이벤트는 처리됨·전역 풀 생존·덕킹 램프 적용과 무관. 근본 원인은 루프 내
   도구로 특정 불가 — 백로그 §1.13에 증거 체인 전체 기록. 부산물로 `snapshotFrame`의
   seek completion 누수 방어 와치독 유지(파리티 스윕 13/13 무회귀).
