...
**판정 신뢰도 경고 (V13 신설)**: 위 `=` 판정 다수는 preview+export 동시 증거 없이 부여됐다. 2026-07-28 핵심 편집 수리가 **메인 Preview가 프로젝트 합성 경로를 쓰지 않고 있었음**(`b398563`)을 드러냈으므로, `=`는 재확인 전까지 잠정으로 읽는다. 재확인 큐: `GAP_ANALYSIS_V13_FUNC_UI_20260729.md` §6.
