...
> 버전: 1.8 / 작성일: 2026-07-06 / 기준 대상: **CapCut 데스크톱(Mac/PC) 2026 버전**
...
| B-I7 | Proxy Mode: Settings > Performance에서 활성화+해상도 선택(720p 권장), 생성 완료 시 클립에 Proxy 배지, export는 원본 사용 | [확인] | **= 도달 (2026-07-30)** — 생성(`ProxyGenerator` 960x540) + 소비(`PlaybackEngine`, `505696f`) + **타임라인 클립 배지**(R5) + export 원본 유지. 실앱 증거 `proxy_generated=1 proxy_badge=idle` / `proxy_playback=1 proxy_badge=active`. 해상도 **선택 UI는 없음**(960x540 고정) — 잔여 ⬇ 요소 |
...
| ~~**프록시 소비 미배선 (B-I7)**~~ | 있음 [확인] | ✅ **해소** — 소비는 `505696f`(preview가 프록시 재생, export는 원본), 타임라인 배지는 R5(2026-07-30). 잔여는 프록시 **해상도 선택 UI**뿐(현재 960x540 고정) | G-11 잔여 축소 |
...
| B-I 동작 | — | **4** | **2** (I1/I4) | I8 (0파일) | **I7 ⬇→= (2026-07-30)** — 소비 배선(`505696f`) + 타임라인 배지(R5), 실앱 `proxy_badge=idle/active` 확인. 해상도 선택 UI만 잔여 |
