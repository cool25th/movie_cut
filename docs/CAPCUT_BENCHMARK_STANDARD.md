...
> 버전: 1.9 / 작성일: 2026-07-06 / 기준 대상: **CapCut 데스크톱(Mac/PC) 2026 버전**
> 변경 이력: v1.9 (2026-07-30) — **B-I7 완결.** 프록시 해상도 선택(480p/540p/720p/1080p, 720p 권장 표시)이 추가되어 기준 문장의 네 요소가 모두 충족됐다. 이 과정에서 두 결함을 함께 고쳤다: export preset이 선택과 무관하게 `AVAssetExportPreset960x540`으로 고정돼 `ProxyInfo.resolution`이 실제 파일과 다를 수 있었고, 프록시 파일명에 해상도가 없어 설정을 바꿔도 기존 파일이 재사용됐다. 1920x1080 소스 실측: 480p→640x360, 540p→960x540, 720p→1280x720, 1080p→1920x1080.
> 변경 이력: v1.8 (2026-07-30) — **B-I7 ⬇→= 승격.** 프록시 소비(`505696f`)에 이어 타임라인 클립 배지(R5)가 배선되어 기준 문장의 세 요소(생성·배지·export 원본 유지)가 모두 충족됐다. 실앱 harness 증거: `proxy_generated=1 proxy_playback=0 proxy_badge=idle` / `proxy_playback=1 proxy_badge=active`. 잔여 ⬇ 요소는 프록시 **해상도 선택 UI**뿐(현재 960x540 고정)이며 별도 소항목으로 추적한다. §6의 프록시 행과 §7 B-I 행을 함께 갱신.
> 변경 이력: v1.7 (2026-07-29) — V13 감사 반영. §6의 확정 격차 2건(블렌딩·컴파운드)을 **G-23/G-24로 등재 완료** 처리하고 실사 근거를 각 행에 고정. **배선 격차 3건 신규 등재**(워드 캡션·프록시 소비·현지화 카탈로그) — 전부 Core/래핑까지만 존재하고 소비 지점이 없다. §7 채점 시트를 실사 근거와 함께 갱신하고, `=` 판정의 신뢰도 경고를 신설했다(7/28 Preview 합성 미사용 발견에 근거).
...
| ID | CapCut 수준 | 신뢰도 | 검증 |
|---|---|---|---|
| B-F3.1 | 자동 캡션: 55+ 언어, 정확도 ~92-95%(명료한 음성), 무료 티어 프로젝트당 10분 제한 | [확인] | STT E2E — **MovieCut은 무제한·온디바이스 강제로 능가가 목표** (온디바이스 인식을 지원하는 로케일에서; 미지원 로케일은 서버 전송 없이 명시적 실패 — S8) |
...
| B-L7 | 클립 표면: 비디오=필름스트립, 오디오=파형, 사진=이미지, 전환=클립 사이 아이콘, 적용 효과 표시 | [확인] | 🟡 비디오 필름스트립·오디오 파형·사진 이미지는 actual app 증거 확보; 전환/효과 표시는 U-02 잔여 |
...
| B-I2 | 스크럽: 룰러 클릭/플레이헤드 드래그에 프리뷰가 프레임 단위 즉시 추종 | [확인] | **🟡 구현+E2E PASS (2026-07-13)**: ruler click/drag, 14pt playhead hit target, pause/coalesce/final exact seek 배선. actual app `requested/playhead/playback=1.250/1.250/1.250`; 실기기 100ms 체감 확인 대기. §6 참조 |
...
| B-I4 | 호버: 에셋 카드 호버 프리뷰, 클립 호버 정보 | [확인] | 🟡 timeline clip cached hover `120×68` actual-app PASS; 에셋 카드 호버 잔여 |
| B-I5 | 자동 저장 + 프로젝트 자동 복구 | [확인] | = 이상 (E2E 상환) |
| B-I6 | undo/redo 깊은 히스토리, 모든 편집 동작 커버 | [확인] | = (스냅샷 undo 무결성 검증) |
| B-I7 | Proxy Mode: Settings > Performance에서 활성화+해상도 선택(720p 권장), 생성 완료 시 클립에 Proxy 배지, export는 원본 사용 | [확인] | **= 완전 도달 (2026-07-30)** — 활성화 토글 + **해상도 선택 4단(480p/540p/720p/1080p, 720p 권장 표시)** + 타임라인 클립 배지 + export 원본 유지. 1920x1080 소스 실앱 증거: 480p→`640x360`, 540p→`960x540`, 720p→`1280x720`, 1080p→`1920x1080`, 각기 다른 파일 |
...
| B-U8 | 워터마크 없음·로그인: CapCut은 **로그인 필요+기능별 페이월**(캡션 10분 제한, Pro $7.99/월) | [확인] | **MovieCut ⬆ (로그인 불필요·무제한·오프라인)** — STT는 온디바이스 강제로 실제 오프라인 보장(S8) |

## 5. 능가 목표선 (MovieCut이 = 가 아니라 ⬆로 가는 지점)

전략(로드맵 §2) 재확인: **B-F4.5/4.6(스코프·Pro출력), B-F6.4(온디바이스), B-U8(무료·오프라인)은 이미 ⬆ — 지킬 것.** B-F5.4(실 EQ)는 v1.1 검증으로 **⬆ 확정**(CapCut 데스크톱 멀티밴드 EQ 부재). 추가 ⬆ 후보: B-F3.1(무제한 온디바이스 강제 캡션 — 지원 로케일 한정, S8), B-F1.4(Photos 직접 드래그 — CapCut 불안정으로 확인, 네이티브 Mac 앱의 구조적 우위 지점), B-F5.3(자동 덕킹 — CapCut은 수동 키프레임이 표준), B-I5/6(신뢰성), FCPXML(G-10, CapCut 부재), 커맨드 팔레트(U-09, CapCut 부재).
...
| 컴파운드 클립(중첩 타임라인) | 있음 [확인] | ❌ — 2026-07-29 실사: `CompoundClip`/`NestedSequence` **0파일** | **G-24로 등재 완료** (v1.9 스펙) |
| 블렌딩 모드(screen/multiply 등) | 있음 [확인] (v1.1 웹 검증) | ❌ — 2026-07-29 실사: `blendMode` 매치 2건이 전부 `CISoftLightBlendMode`/`CIOverlayBlendMode` **내부 필터명**, 사용자 노출 0 | **G-23으로 등재 완료** (v1.9 스펙) |
| 캡션 VTT/ASS export | 있음 [확인] | SRT만 — `WebVTT`/`AdvancedSubStation` 0파일 | F-13 확장 소항목 |
| **워드 캡션 데이터 미배선 (B-F3.3)** | 있음 [확인] | 🟡 **배선 격차** — `wordTiming` Core 4파일 / App **0파일**. STT가 뽑아 모델에 저장하나 렌더·편집 UI가 소비 안 함 | G-01에 귀속, V13 §3-B |
| ~~**프록시 소비 미배선 (B-I7)**~~ | 있음 [확인] | ✅ **완전 해소** — 소비 `505696f` + 타임라인 배지 R5 + **해상도 선택 4단**(2026-07-30). B-I7 잔여 없음 | G-11 해당 항목 종료 |
| **현지화 카탈로그 부재 (B-U6)** | 있음 [확인] | ❌ **래핑만** — `NSLocalizedString` 8파일이나 `.lproj`/`Localizable.strings`/`.xcstrings` **0개** → 실질 영어 전용 | U-06에 귀속, V13 §3-B |

## 7. 채점 시트 (감사 세션이 갱신)

> V13(2026-07-29) 갱신. 이전 스냅샷은 2026-07-06 V12 시점.

| 영역 | ⬆ | = | ⬇ | ❌ | V13 실사 근거 |
|---|---|---|---|---|---|
| B-F1 미디어 | 1 (F1.5 ProRes 포함 시) | 5 | 1 (F1.4 실기기) | — | G-15 상환 — `ImageVideoRenderService`가 Playback/Export/Preview 3곳 배선 |
| B-F2 타임라인 | — | 5 | 1 (F2.4 커브 프리셋 UI — `SpeedCurvePreset` 0파일) | F2.3 컴파운드 → **G-24 등재** | `=` 5건은 **preview+export 재확인 대기**(§0 6항) |
| B-F3 텍스트·자막 | 후보 F3.1 | 2 | 2 (F3.2 VTT/ASS 0파일, F3.4 템플릿 14종) | **F3.3 워드 캡션** | F3.3은 **배선 격차** — Core 4파일/App 0 |
| B-F4 색·효과 | **F4.5/F4.6** | 2 | F4.1(UI)/F4.2/F4.3 | F4.4 블렌딩 → **G-23 등재** | 볼륨: 전환 12·이펙트 18·스티커 22 |
| B-F5 오디오 | F5.4 | 2 | F5.1 (SFX 12개) | F5.2 보컬분리 | `VocalSeparationService` **dead code 121줄** 확인 |
| B-F6 AI | F6.4 | 1 | F6.2 | F6.3 | F6.2 관련 **미배선 368줄**(`ClaudeEditingProvider`+`AIEditingProvider`) |
| B-L 화면 | — | 4 | 2 (L3/L7) | **L2 홈** | 홈/프로젝트목록 **0파일** — `WindowGroup`이 곧장 편집기 |
| B-I 동작 | — | **4** | **2** (I1/I4) | I8 (0파일) | **I7 ⬇→= 완결 (2026-07-30)** — 소비(`505696f`) + 배지(R5) + 해상도 선택 4단. 1080p 소스에서 480p→640x360 … 1080p→1920x1080 실측 |
| B-U 사용성 | **U8** | 3 | 2 (U3/U5) | **U6 현지화** | U6 = 번역 카탈로그 **0개**, 래핑만 존재 |

**판정 신뢰도 경고 (V13 신설)**: 위 `=` 판정 다수는 preview+export 동시 증거 없이 부여됐다. 2026-07-28 핵심 편집 수리가 **메인 Preview가 프로젝트 합성 경로를 쓰지 않고 있었음**(`b398563`)을 드러냈으므로, `=`는 재확인 전까지 잠정으로 읽는다. 재확인 큐: `docs/GAP_ANALYSIS_V13_FUNC_UI_20260729.md` §6.
