...
- [ ] 🟡 **G-04 타임라인 필름스트립+호버 스크럽 (P1)** — 2026-07-16 Inc 1~5 구현 완료. 실제 `TimelineView` request→cache/decode→publish→UI consumer에 production signpost를 연결하고, actual-app 20/40/80/160px/s에서 bucket `0/1/2/3`, time-varying density `0.247/0.423/0.697/1.394 frame/s`, image/audio/text 표면 보존을 확인했다. 10분 4K synthetic low-bitrate fixture의 1분 간격 seek에서 RSS `221.5→227.7MB`(delta `+6.2MB`, threshold `+100MB`), decoded cache peak `8,816,640B/128MB`, height `41`을 기록했다. MainActor scheduling-gap proxy는 `n=1218`, p95 `2.096ms`를 통과했으나 max `67.106ms`, `>16.6ms=4`여서 AC1의 초과 0건과 실기기 display-cadence 확인이 남는다. 따라서 G-04/UB-V4 완료는 선언하지 않는다.
