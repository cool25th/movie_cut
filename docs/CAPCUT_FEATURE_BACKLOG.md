...
V10/v1.4 재평가 기준 우선순위: G-12 자동 상환은 **#9 챕터/비트 마커 메타데이터까지 완료**했고, **G-02 Inc 3**(`ColorGrade` 저장 필드 + HSL/curve renderer chain + golden/E2E)도 완료했다. 따라서 다음 자동 선택 항목은 **U-08 회귀/지표 인프라**이고, 그 다음은 G-01 Inc 2(active-word caption renderer) 순서다. #11/#12는 fixture 제작 증분으로 분리해 병행 슬롯에서 다루고, #13/#14는 수동 검증 대기로 자동 선택에서 제외한다. 레거시 P1 정적 계약 기준의 다음 1순위는 F-01 실기기 검증이며, V10 신규 루프와 병행한다.
