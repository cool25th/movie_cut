...
| CA-19 | 타임라인 **가이드라인**(드래그 기준선) + 눈자 밀도 감사 | 소형 | **iOS 스냅+가이드 완료(2026-08-26)** — 드래그 종료 시 다른 클립 가장자리·플레이헤드·0에 스냅(14pt 반경) + 액센트 가이드라인 표시(Mac 패리티). Mac은 이미 완성. 부수: Mac trim 제스처 `snappedTime` 미정의 빌드 오류 수정. 잔여: 밀도 감사 보고(장편 분 단위) | iOS 드래그 스냅+가이드 실측 + Mac 빌드 복구 |
...
| CA-22 | 프록시 자동 생성 — 임포트 시 백그라운드 | P2 | **1차 완료(2026-08-26)** — `PlaybackSettings.autoGenerateProxyOnImport`(기본 true·Codable 하위호환) + `scheduleAutoProxyGeneration`: 비디오 임포트 후 fire-and-forget Task로 프록시 생성(중복 방지 Set·thermal critical 스킵). 잔여: 설정 UI 토글·진행 취소·재개 | 백그라운드 생성 E2E·thermal 상호 정책 |
