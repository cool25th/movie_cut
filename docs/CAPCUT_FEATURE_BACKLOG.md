...
- [ ] 🟡 **G-04 타임라인 필름스트립+호버 스크럽 (P1)** — 2026-07-15 Inc 1~4 완료. Inc 1~3의 실제 `TimelineView` viewport-aware lazy/cancel/stale/fallback consumer를 보존하고, Inc 4는 clip-local `onContinuousHover`를 sourceRange/timeline duration/playback mapping으로 환산해 **published cached frame만** 최근접 선택하는 120×68 preview+time label을 추가했다. Focused behavioral **18/18 PASS**; actual app UI-consumer E2E는 기존 `frames/digests/times=32/32/32`, visible-only/cancel/stale/fallback 증거와 함께 hover requested/actual `11.000/11.000`(error `0.000s`), digest membership, exit/cache-miss/unsupported hidden, request/generation delta `0/0`을 기록해 **AC4를 충족**했다. Caveat: Inc 5 signpost/perf·메모리와 AC1~3, image/audio/text 기존 렌더 보존 전수 증거인 AC5가 남아 G-04/UB 완료가 아니다. `[진행중] 다음 증분: Inc 5, 시작점: App/MovieCutMac/Media/TimelineFilmstripStore.swift:121`.
