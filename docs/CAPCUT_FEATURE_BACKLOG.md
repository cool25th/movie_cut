...
**V10 기능+UI 재감사(2026-07-05, 기준 `738f4ce`)**: 최신 분석 문서는 `docs/GAP_ANALYSIS_V10_FUNC_UI_20260705.md`다. G-12 #7 상환을 독립 검증(E2E 스크립트/하니스 훅/실측치 실재 + `swift build`/361 tests PASS)해 **9/14 확정**. 신규 판정: **dead-value 4계열 누적** — `CurveEvaluator`·`HSLCubeBuilder`(App 참조 0), `CurvePoint`/`HSLBand`(`ColorGrade` 저장 필드 미편입), `wordTimings`(렌더러 소비 0) → 스펙 v1.4 A6 보강으로 G-12 원장 등재. **S0 게이트 완화(스펙 v1.4)**: 자동 상환은 #9까지, #11/#12는 fixture 증분(#11a/b, #12a/b) 세분화, #13/#14는 수동 검증 대기 분리 — 이후 자동 선택은 **G-02 Inc 3 → U-08 → G-01 Inc 2** 순.

**CapCut 대비 완성도 재평가(2026-07-05 loop-6, 기준 `fe9b062`, 과거 기준)**: 분석 문서는 `docs/GAP_ANALYSIS_V9_FUNC_UI_20260705.md`다. 52주기식 자가보고 97%는 계속 폐기한다. G-12가 9/14로 증가(#1 EQ, #2 NR, #3 덕킹, #4 모션 트래킹, #5 옵티컬 플로우, #6 텍스트 애니메이션, #7 타이틀 템플릿, #8 플랫폼 프리셋, #10 오디오 추출)하면서 체감 완성도는 68~74%로 소폭 상향 가능하나, 아직 “능가” 선언은 금지다. 미완 핵심은 (1) G-02 HSL/커브 렌더 체이닝+저장+UI+iOS, (2) G-01 caption style model+active-word renderer+preview/export+iOS, (3) G-12 잔여 5개(#9/#11/#12/#13/#14), (4) G-04 필름스트립, G-05 보컬분리/보이스FX, G-06 이징 UI, G-09 iOS 본대, (5) U-01/U-02/U-04/U-05/U-06/U-08/U-09 UI 표면이다.
...
V9 재평가 기준 우선순위: 엄격한 `/surpass` S0 게이트를 적용하는 세션은 G-12 잔여 중 **#9 챕터/비트 마커 메타데이터**를 다음 자동 선택 항목으로 집는다. 이후 #11 배경제거 실인물, #12 자동 리프레임 실영상 추적, #13 iCloud 2기기 충돌, #14 Photos 앱 드래그 순서다. 기능 체감 ROI만 보면 G-02 Inc 3(`ColorGrade` 저장 필드 + HSL/curve renderer chain + golden/E2E)가 여전히 최우선 후보이며, 이유는 `CurveEvaluator`/`HSLCubeBuilder`가 App-visible 경로 없이 남아 있기 때문이다. UI 트랙은 U-08 회귀/지표 인프라를 선행한다. 레거시 P1 정적 계약 기준의 다음 1순위는 F-01 실기기 검증이며, V8 신규 루프의 S0 검증부채 상환과 병행한다.
