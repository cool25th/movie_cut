...
| CA-02 | 파리티 허용 오차 등급 수치 확정(Exact/Tolerance/Perceptual) | P0 | **즉시 실행 가능(Q7 확정: Tolerance MAD ≤ 2.0 유지 — `DECISIONS_20260822.md`)** | VERIFICATION_STANDARD §2 등급별 수치 확정 + 골든 재판정 기준 문서화 |
| CA-03 | 미디어 관리·프로젝트 생존성 감사(재연결·누락·손상·마이그레이션 실패 경로·디스크) | P0 | **승인 완료(Q11) — 방향 문서 §3 반영 후 실행** | 감사 보고 + 발견 결함의 P0 버그 등록 |
| CA-04 | 입력 포맷 호환 매트릭스(VFR·10bit·Log·혼합 fps/sample rate·rotation) | P0 | **승인 완료(Q11) — 방향 문서 §3 반영 후 실행** | 매트릭스 작성 + 최우선 회귀(혼합 미디어 sync·색 유지) 실측 |
| CA-05 | 실패·복구 UX 매트릭스(15 실패 시나리오 × 무손실/원인/재시도/이어하기/임시파일) | P0 | **승인 완료(Q11) — 방향 문서 §3 반영 후 실행** | 매트릭스 + 결함 우선순위화 |
| CA-06 | 접근성 핵심 경로 매트릭스(임포트→편집→출력, VoiceOver 등) | P0 | **승인 완료(Q6·Q11 — 범위: 핵심 경로+키보드)** | 매트릭스 + 차단 결함 등록 |
...
| CA-26 | LUT export(.cube 저장) — 그레이딩→LUT 저장 경로 부재(2026-08-23 전역 확인: writeLUT/exportLUT 0건, import만 존재). W3/W4 색 워크플로 완결·표준 포맷이라 오프라인 원칙 무관 | 소형 | **즉시 실행 가능(2026-08-23 승인 — FCP 확정판 v4)** | .cube 직렬화 round-trip 골든(임포트 LUT 재저장 무손실) + 현행 파라미터→LPT 변환 범위 명시 + export 진입점(U) |
| CA-27 | Timecode 직접 입력 — `PreviewPanel` 표시 전용(2026-08-23 확인: `timecodeString` value 필드만, 입력 0건). 키보드 완결성·정밀 탐색(Q6 핵심 경로 정합) | 소형 | **즉시 실행 가능(2026-08-23 승인 — FCP 확정판 v4)** | 시간 포맷 파싱(hh:mm:ss:ff·초) → playhead 이동 골든 + VoiceOver 라벨 + 오류 시 명시적 실패 |
| CA-28 | RGB 파레이드 스코프 — `parade` 0건(2026-08-23 확인, 벡터스코프는 존재). `ScopeViews` 확장 소형 | 소형 | **즉시 실행 가능(2026-08-23 승인 — FCP 확정판 v4)** | 파레이드 렌더 골든 + 인스펙터 노출(U) + 기존 스코프 회귀 |

**실행 규칙**: CA-01·02·08·12·14·15·17·19·22·24·26·27·28는 즉시 실행, CA-03~06은 방향 문서 §3 반영 후 실행, CA-18·21은 측정 단계만, CA-20·23·25는 등록 완료(실행 조건 도달 시), 나머지는 승인 대기. AI 음성(TTS 보이스 확장)은 **등록 보류(2026-08-23 승인 — 베타 반응 후 재상정)**. 루프 회차 보고에는 '승인 대기' 항목을 항상 나열한다.
