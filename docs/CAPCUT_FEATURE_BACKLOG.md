...
- [ ] 🟡 비파일 드래그 소스(사진/브라우저, F-01) (P1) — Core `DragDropHandler.loadExternalMediaURLs`가 fileURL/movie/image 페이로드를 처리(`loadFileRepresentation`→data fallback→`MovieCutImports` 임시 디렉토리), 타임라인/라이브러리 onDrop에 `.movie`/`.image` 추가. `ExternalMediaDropTests` 6개가 실제 NSItemProvider 페이로드로 행동 검증. Caveat: 실기기 드래그(사진→타임라인, Safari→타임라인) 확인 잔여 — DoD §1.3에 따라 ✅ 보류.
