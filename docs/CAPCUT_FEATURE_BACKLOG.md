...
- [ ] 🟡 **G-04 타임라인 필름스트립+호버 스크럽 (P1)** — 2026-07-15 Inc 1~3 완료: `AVAssetImageGenerator` tile-center 생성·128MB/256-key bounded cache를 실제 macOS `TimelineView`에 연결했다. named horizontal viewport와 실제 scroll viewport width로 반 화면 prefetch 범위만 계획하고, offscreen skip, geometry/zoom/source/media request 변경·view disappear 취소, generation token stale reject를 적용하며 기존 단일 썸네일 폴백을 유지한다. Focused behavioral **11/11 PASS**; actual app UI-consumer E2E는 `frames/distinct_digests/distinct_times=32/32/32`, `visible_span=21.600/30.000`, `visible_count=32/67`, `offscreen_skipped/cancelled/stale_rejected/fallback_before_ready/fallback_after_cancel=1`을 기록했다. Caveat: Inc 4 hover, Inc 5 signpost/perf·메모리, AC3 4단계 실제 UI 밀도 및 AC5 비디오 외 시각 회귀 측정이 남아 G-04/UB 완료가 아니다. `[진행중] 다음 증분: Inc 4, 시작점: App/MovieCutMac/TimelineView.swift:975`.
...
- [ ] ❌ **G-18 카드 문서 모델+편집기 (P0)** — 전용 모델/command/UI 0건. 목표: 카드 추가·복제·삭제·순서 변경 각각 ≤2클릭, 1:1·4:5·9:16 전환 무파손, 텍스트 더블클릭 1회 인라인 편집. SC-C1 ≤10분·막힘 0/P-반복 ≤5분은 기능 완성 후 사용자 확인.
