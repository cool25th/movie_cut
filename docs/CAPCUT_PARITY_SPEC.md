...
> **실기기 GUI 검증 패스(2026-06-14)**: 최신 빌드 앱에서 Finder→타임라인 실드래그로 **드래그앤드롭 클립 생성**(Video 1 레인에 클립), **실제 3.0s duration**(Export Summary "~7.6 MB for 3.0s"), **썸네일 필름스트립 + preview 프레임 렌더**를 라이브 확인. 앱은 이번 세션 전체 변경(F-07~F-23)을 포함해 정상 실행. 나머지 🟡 항목(LUT/크로마키/덕킹/자막/배경제거 등)은 결정론적 픽셀·행동 단위 테스트로 뒷받침되며, 개별 GUI 확정은 후속 패스로 남김.
