...
#### 현재 상태 (실사)
- 2026-07-18 Inc 1~5 + 잔여 AC1 구현/자동 검증 완료. 실제 `TimelineView` 비디오 표면은 viewport-aware 시간가변 스트립과 cached-frame-only 호버를 소비하고, request/cache lookup/decode/cache insert/publish/UI consumer lifecycle은 production `os_signpost`로 계측된다. AC1은 scheduler wakeup이 아니라 actual main-thread request/publish/consumer update/AppKit draw 구간을 재서 2회 모두 16.6ms 초과 0건을 강제 통과했다. 표시 presentation cadence/실기기 스크롤 체감은 이 수치가 주장하지 않는 사용자 확인 caveat다.
...
- 2026-07-16 Inc 5 / AC1 `[당시 진행중]`: 실제 3분 1080p `TimelineView` zoom+scroll 중 1ms MainActor scheduling-gap proxy(표시 FPS 아님) `n=1218`, `p95=2.096ms`(enforced ≤16.6ms), `max=67.106ms`, `>16.6ms=4`; stable p95 기준은 통과했지만 AC의 **초과 0건**은 당시 미충족이었다.
- 2026-07-18 잔여 AC1 **충족**: 위 1ms sleep/wakeup 값은 filmstrip 작업 시간이 아니라 OS scheduling/app lifecycle 지연을 섞은 artifact로 진단했다. 실제 3분 1080p actual app에서 20/40/80/160px/s 각각 zoom 1회+real `ScrollViewReader` mutation 3회를 구동하고, signposted main-thread request/publish/consumer update/actual AppKit draw를 **필터·clamp 없이 전수** 집계한다(정확한 budget `16,600,000ns`). exact-final 독립 2회: run 1 `n=66/request=17/publish=17/update=16/draw=16/identities=17`, `p95=0.719ms`, `max=1.122ms`, `>16.6ms=0`; run 2 동일 coverage, `p95=0.402ms`, `max=0.435ms`, `>16.6ms=0`. 스크립트는 초과 수가 1 이상이면 실패한다. 이는 filmstrip-attributable main-thread work 증거이며 display FPS 주장은 아니다.
...
- `[완료] G-04 AC1~AC5 자동 검증 완료. [사용자 확인 대기] 실기기 display-cadence/스크롤·호버 체감(자동 main-thread work 수치는 표시 FPS가 아님).`
...
- 2026-07-17 Inc 1 완료: optional `Project.cardDocument`와 `CardDocument`/format/page/element/normalized geometry 및 G-19 경계의 최소 `CardMasterStyle` persisted shape, Add/Duplicate/Delete/Move/Update command를 기존 `EditorSession` snapshot undo에 연결했다. focused `swift test --filter CardDocumentCommand` **15 tests / 1 suite PASS**(실제 pre-card JSON의 `cardDocument == nil` 및 timeline/export 값 보존, round-trip, fresh duplicate IDs/content, 결정적 순서, normalized frame, 각 동작 single-step undo/redo, missing/invalid/last-page fail-closed). 이는 Core 기반 증분이며 UB-C1/C3/C4·SC-C1 완료 증거가 아니다.
- 2026-07-18 Inc 2 완료: `CardEditorView`의 selectable rail/add·duplicate·delete/drag reorder/1:1·4:5·9:16 picker가 전부 `EditorViewModel`→`EditorSession.dispatch`로 배선됐고 focused `CardDocumentCommand` **17/17 PASS**; 실제 앱 accessibility 조작으로 page 2 선택→duplicate(`4 pages`, selected `3/4`)→9:16 전환을 확인하고 `artifacts/ui/g18_inc2_card_editor_runtime.png`를 캡처했다. XCUITest는 빌드/launch 후 호스트의 admin `Enable UI Automation`에서 `0/1` 차단(`artifacts/G18Inc2CardEditor.xcresult`); drag 자동화는 미수행이다. `[진행중] 다음 증분: Inc 3, 시작점: App/MovieCutMac/CardNews/CardCanvasView.swift 신규 normalized canvas + EditorViewModel.updateCardElement(...) → UpdateCardElementCommand`.
- 2026-07-18 Inc 3 완료: `CardCanvasView`가 normalized source-of-truth로 3규격 요소를 렌더하고 move/resize/인라인 텍스트/atomic 이미지 교체를 gesture당 command 1회로 확정하며 focused `CardDocumentCommand|CardLayout` **26/26 PASS**(stable ID·single-step undo/redo·3규격 round-trip error ≤0.001); actual-app accessibility 증거는 `artifacts/ui/g18_inc3_card_canvas_runtime.png`에 기록했다. XCUITest는 호스트 admin UI Automation 권한 차단을 PASS로 주장하지 않는다. `[진행중] 다음 증분: Inc 4, 시작점: UITestHarness.swift의 MOVIECUT_UITEST_CARD_EDITOR save/reload dump`.
- 2026-07-18 Inc 4 완료: actual-app `MOVIECUT_UITEST_CARD_EDITOR=1` 하니스가 동일 `EditorViewModel` command 경로로 add(2)/duplicate(1)/delete(1)/reorder(1)를 수행해 최종 5페이지(순서 ID 일치), square→portrait→story 전환, inline double-click=1 회 텍스트 수정+undo 1회 복원+redo 1회 재적용, 실제 `ProjectStore` save→fresh session reload를 구동했고 dump는 `complete=true/error=none/maxNormalizedFrameError=0/saveReloadEqual=true/freshSessionReloaded=true/savedProjectBytes=6098`를 기록했다. UB-C1(add/duplicate/delete/reorder 각 ≤2), UB-C3(3규격 observed, frame error 0≤0.001), UB-C4(inline 1회 진입+undo 복원)가 자동화 측정값으로 충족됐다. `scripts/run_e2e_export.sh` 상설 체크가 `PASS: card editor save/reload E2E`로 전체 `E2E check OK`와 함께 통과. SC-C1(≤10분/막힘 0)은 기능 전제 완성 후 사용자 화면 녹화로 판정하므로 `[사용자 확인 대기]`를 유지한다. `[진행중] 다음 마일스톤: G-19 카드 템플릿 세트 + 마스터 스타일, 시작점: Sources/MovieCutCore/Models/CardTemplateSet.swift 신규 + CardEditorView 템플릿 피커`.
