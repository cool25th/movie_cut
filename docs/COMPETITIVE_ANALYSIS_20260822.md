...
전환 11종 Mac✅/iOS 렌더 미배선(M1) — `TransitionType` 구체 케이스 기준(`.none`·범주 라벨 제외 11개, 2026-08-22 코드 확인). CapCut 템플릿 바이럴 마켓은 복제하지 않음 — 대응은 G-28 탐색 품질(완료)+**N2 오토스타일**. 애니메이션 스티커(N3)는 `.moviecutpack` v0와 세트(3단계). 타임라인 눈자·가이드라인 소형.
...
방향 문서 §3 고정 순서와 충돌하는 신규 편입(⑤⑥⑨⑩)은 사용자 승인 후 반영(Q11). ⑦(Tolerance 수치)은 MC-05·Q7 기존 결정의 이행, ⑧(오프라인 차단 테스트)은 §1.3 MC-02②③ 기존 P0의 이행으로 신규 기획 편입이 아니다.
...
|---|---|---|---|
| 트림/스플릿/리플 | ●●●●● | ●·◐·◐·◐·○ | iOS 트림 = 툴바 버튼 존재하나 no-op + 핸들 UI 부재(U❌) |
