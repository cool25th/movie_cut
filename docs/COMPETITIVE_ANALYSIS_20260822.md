...
| # | 격차 | 내용 | 매핑 |
|---|---|---|---|
| M1 | ~~전환(two-source) 렌더 부재~~ **해결(2026-08-26, BUG-IOS-09)** — iOS 2슬롯 교차+백타이밍+makeTransitionEffects 포팅, 플랜 프레임 적→혼합→청 실측. 12종 UI 진입 잔여 | 완료 — UI 진입만 잔여 |
| M2 | ~~autosave/복구 부재~~ **해결(2026-08-24, BUG-IOS-02+UX-REC-02)** — 커밋마다 ProjectStore autosave+복구 유지/버림 선택 UI. | 완료 |
| M3 | 출력 옵션 | 컨테이너는 mp4/mov planner 해석(2026-08-28 리뷰 P0 수정·실측) — **해상도/화질 프리셋 UI 잔여** vs 상대 4K·화질 선택 | G-09 Inc3 4순위(축소) |
| M4 | ~~오디오 도구~~ **해결(2026-08-26, BUG-IOS-10+덕킹 램프)** — audioMix 램프(볼륨·페이드)·그래프 AAC 오디오·덕킹 적용 경로 실측. | 완료 |
...
Mac: H.264/HEVC/ProRes 422·4444·1-200Mbps·챕터·소셜 프리셋 5종(ffprobe)·프록시 4단계+thermal. iOS 컨테이너는 planner 해석(mp4 기본 — 2026-08-28 리뷰 P0 수정·실측), 해상도/프리셋 UI 잔여(M3 축소). FCP 12.3이 백그라운드 렌더 기본 OFF로 전환한 교훈: 성급한 백그라운드 렌더는 렌더 파일 팽창 부채 — F9는 2~3단계. **성능 우위 주장은 §7 하니스 완료 전 보류.**
...
| MC-07 | 프리뷰=출력(Exact/Tolerance) | **내부 증거 상시 게이트화(2026-08-31 재감사)** — 19시나리오 코어 파리티 스윕(크로스디졸브 재편입 포함)이 `run_core_editing_parity.sh`+nightly CI로 상시 가동·MAD≤2/시나리오. 잔여: 스냅숏 스테일 공급 클래스(BUG-CA12-01·BUG-ACC-06) 4시나리오 간헐 FAIL — 루트 수정 후 18/18×3. 상태표는 `LOOP_STATE_REPORT.md`(생성 전용·STAB-08) |
