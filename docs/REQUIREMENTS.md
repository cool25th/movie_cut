...
| 2026-08-22 | **검증 정의 확정 + 경쟁 분석 통합 + 문서 체계 정리** — ① 완료 판정을 E/U/P/X/D/S 6단계로 정의(UI 미노출=미구현, 경쟁 비교는 S만), ② 프리뷰=출력을 Exact/Tolerance/Perceptual 3등급 허용 오차로 재정의(외부 문구 "보이는 대로 출력"), ③ 성능 수치 정의(RTF=경과/출력길이, seek는 모델 계산과 입력→표시 분리), ④ 오프라인 주장의 증명 체계(Mac entitlement 확인·iOS 코드감사 완료·차단테스트 P0), ⑤ 경쟁 분석을 `COMPETITIVE_ANALYSIS_20260822.md`로 통합(YT Create·CapCut·FCP 12.3), ⑥ 출시 기본기 신규 P0 후보(미디어 관리·입력 포맷·실패/복로 UX·접근성·가격 결정 — 방향 문서 반영은 승인 대기). 구형 문서 9종 archive 이동 |
| 2026-08-23 | **CA-17~19 등록 승인(전체 승인)** — ① CA-17 자막 sidecar 검증(VTT/SRT 구현 이미 존재 — 737c036, 실제 플레이어 로드 D·iOS 진입만 잔여), ② CA-18 화자 분리 게이트형 연구(임계값 사전 등록: 혼동율 ≤10%·RTF ≤0.5 — 측정 단계만 승인), ③ CA-19 타임라인 가이드라인+눈자 밀도 감사(눈자는 이미 존재), ④ G-09 Inc3 UI 범위 명문화 채택(PLATFORM_PARITY_MATRIX §4), ⑤ AI 음성 등록 보류. 상세: `CA_REGISTRATION_PROPOSAL_20260823.md` v2(외부 검토로 CA-17·19 전제 오류 정정 후 승인) |
...
### 13.15 제품 결정 12건 확정 (2026-08-22, Q1~Q12)

**요구사항.** 경쟁 분석(`COMPETITIVE_ANALYSIS_20260822.md`) Part 8의 열린 결정 12건을 사용자가 4지선다로 확정했다(`DECISIONS_20260822.md`가 결정 원천). 본 절과 충돌하는 이전 항목은 이 절이 우선한다.

1. **페르소나**: 1인 쇼츠 크리에이터(세로형·한국어 자막·음악 컷 중심) — W1/W2 대표 작업이 우선순위의 척도.
2. **가격 모델**: 일회성 구매 + 유료 메이저 업데이트(FCP식)·Universal Purchase 동반. 구체 가격은 별도 결정.
3. **macOS 14 지원 유지** — 구형 Mac 시장은 FCP(15.6+ 요구) 대비 차별화.
4. **N6 뷰티·F-24 업로드 보조: 비목표 확정** — 오프라인 원칙(외부 전송 없음) 최우선.
5. **자막**: 한국어+영어 우선 실증(WER/CER 측정 후 목표치 확정), 네트워크 폴백 없음.
6. **접근성 게이트**: 핵심 경로(임포트→편집→출력, VoiceOver+키보드) — 타임라인 UI 후조립 불가 경고에 따라 현 단계 편입.
7. **파리티 Tolerance 수치**: 현행 MAD ≤ 2.0 유지(세분은 데이터 확보 후).
8. **성능 수치**: 출시 직전 1회 재측정 후 공개.
9. **경쟁 실측**: 분기 고정(버전·횟수·범위 선언). 10. **릴리스**: 분기 1회.
11. **P0 편입 승인**: 미디어 생존성 감사·입력 포맷 매트릭스·접근성 매트릭스 — 방향 문서 §3 반영 후 실행.
12. **공식 지원 상한: 프로젝트 60분** — 60분 드리프트·null test 통과 범위와 정합. 사양에 명시.

### 13.17 FCP 확정판 v4 — CA-26/27/28 등록·Auditions 보류·정정 2건 (2026-08-23)

신규 부재 4건 코드 확인(LUT export·Timecode 입력·Auditions·연결 스토리라인 전역 검색) 후: CA-26 LUT export·CA-27 Timecode 직접 입력·CA-28 RGB 파레이드 등록 승인(즉시 실행), Auditions는 CA-16 P2 묶음 보류 편입. 연결 스토리라인은 패러다임 차이로 격차 제외. 정정 2건 기록: ① "보컬분리 dead code"는 허위(`VocalSeparationService`+`InspectorVocalSection` UI 진입 존재 — 부재는 ML 음성 향상 쪽), ② CA-09/10은 승인 대기 유지(DECISIONS §"조건 변동 없음").

### 13.16 CA 큐 신설 등록 6건 + 벡터스코프 정정 (2026-08-23)

`CA_REGISTRATION_PROPOSAL_20260823.md` v3의 사용자 전체 승인(FCP 축·사용성 축·정정)에 따라 백로그 §0.5.1에 등록했다. **CA-20** roles+타임라인 인덱스(방향 문서 §3 반영 후 실행) · **CA-21** Edit Detection(측정 단계만 — 임계값 사전 등록제) · **CA-22** 프록시 자동 생성(즉시 실행 가능) · **CA-23** 프로젝트 스냅샷(등록, 실행 별도) · **CA-24** 한국어 UI 커버리지 100%(즉시 실행 가능) · **CA-25** 온보딩·샘플 프로젝트(방향 문서 §3 반영 후 실행). **정정**: 벡터스코프는 이미 구현·Mac 배선(`InspectorEffectsSection.swift:200`)으로 CA-16 묶음 및 경쟁 분석 P2 목록에서 제거 — U 확보, P/X 미검증.
