...
백로그 §3 기준 미완료 P1 항목. 전환효과 two-source compositor 통합, 썸네일/프록시 생성, speed ramp preview static contract 검증, 텍스트 스타일 편집 UI, 보이스오버 실녹음, 페이드 duration 편집 UI는 완료됐으므로 다음 우선순위는 마그네틱 타임라인 / 클립별 zIndex:

| # | 작업 | 시작점 |
|---|---|---|
| 1 | **마그네틱 타임라인 / 클립별 zIndex** — 구조 변경 수반, 단독 배치로 진행 권장 | `TimelineView.swift`, Core `Track`/`Clip` 모델 |
| 2 | **텍스트 템플릿/타이틀 프리셋** — Core template 존재 여부와 실제 Inspector/Canvas 적용 경로를 확인해 단독 배치로 진행 | `Inspector/InspectorBasicSection.swift`, Core text/template 모델 |
