...
M2 진행 현황: F-07은 2026-06-11 targeted pass로 fade-through-black boundary pixel fixture와 Inspector verification guard가 추가됐다. 이는 targeted transition confidence only이며 export golden/device playback/full-suite/release-ready claim은 금지한다. 다음 F-07 산출물은 deterministic golden export sample(output path + hash). **F-11 캔버스 배경은 구현+테스트+배선 완료(🟡, 실기기/픽스처 잔여 — 스펙 F-11 검증 기록 참조)**. 다음 후보: F-09 외부 LUT(InspectorEffectsSection 충돌은 이번 F-07 커밋 후 해소), F-10 크로마키 스포이드, F-08 배경제거 E2E.
