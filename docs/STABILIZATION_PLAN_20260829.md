...
| STAB-02 | P0 | **완료(2026-08-29 펌프 병렬화 + 2026-09-03 취소 E2E 완결)** — 1차: 태스크그룹 동시 pump(iOS RENDER-02 패턴 포팅) + 펌프 실패 시 양 reader/writer 즉시 해체 + writer `.cancelled`→CancellationError + 활성 세션 추적·cancelExport 배관. 2차(취소 E2E — 하니스 노브 `MOVIECUT_UITEST_EXPORT_CANCEL_MS`·`scripts/run_e2e_cancel.sh`·`ExportCancelMidFlightTests`): E2E가 **제품 결함 2건 적발·수리** — ① 취소된 writer는 `requestMediaDataWhenReady` 핸들러를 영구 재호출하지 않아 양 펌프 continuation 누수→익스포트 영구 파킹(93s 와치독 킬·continuation MISUSE 로그 실측 — "취소된 리더의 copyNext→nil로 재개" 믿음은 취소 케이스에 거짓) → 펌프에 writer 상태 취소 폴러+`PumpContinuationOnce` once-guard 추가 ② 취소된 writer에 `finishWriting`→`NSInternalInconsistencyException`(status 4) 프로세스 사망(reader 해체 경쟁 시 그룹이 정상 복귀하는 경로) → finishWriting 전 cancelled/failed 선검사. 부수 함정 2건 기록: 단발 취소 노브는 오디오 그래프 렌더 중(writer 등록 전) 착지해 no-op(반복 발사로 해결 — cancelExport는 등록 전 no-op여도 isExporting=false 세팅), mediaLibrary 딕셔너리 키≠asset.id(구조체 필드)면 그래프가 assetMissing 사망(키=asset.id는 임포트 파이프라인 불변식 — 직접 Project 구성 테스트는 id 명시 전달 필수) | 루프 | **취소 E2E 게이트 3/3 PASS(정직 3조건: UITEST_DONE+error≠none+부분출력 부재 — 구 판정은 파킹 앱도 파일 부재로 가짜 PASS)·유닛 2/2×3회·Mac 유닛 54/54(UI 러너 행업은 MACUI-01 계열 환경 판정)·W 스모크 5/5(29/29)·verify_gate 5/5.** 원 1차 측정 유지: W4 3회 연속 + W 5/5×3회 + 고아 0 |
...
- **Phase 3+ — 베타 준비 이후** (리뷰 권장 2~4단계 전항 원장화 — STAB 창구 종료 후 이 체크리스트가 복귀 큐):
  - **3-A 베타 준비(리뷰 2단계)**: 실기기 3/3(G-27 — 현재 1/3·사용자) · 30~60분 프로젝트 soak 2회 연속(✅ 2026-08-29 달성) · 백그라운드/디스크 부족/취소/재열기/외장 미디어 검증 매트릭스(✅ **측정 완결 2026-09-04 — `PHASE3A_VERIFICATION_MATRIX_20260904.md`**: 취소 Mac 3/3+iOS 2/2·재열기 베타 시나리오·외장 미디어 프리플라이트 테스트·디스크 풀 **실ENOSPC 게이트 2/2**(`run_diskfull_gate.sh` — 1MB DMG에 ProRes·`error=디스크 가득참`·목적지 클린). 잔여는 전부 실기기 관찰(iOS 서스펜션·물리 언플러그)) · signed archive 실증 · 검토 가능한 PR 단위 push 상시 유지(✅ 체계화 — 스택 PR) · 10~30명 베타 중대 데이터 손실 0건.
