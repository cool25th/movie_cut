...
>
> **PARITY-TOL-01 해결(2026-08-28, 승인된 권고안 (a) 실행)**: 파리티 픽스처를 캔버스 정합 ≥720p
> (1440x1080 4:3 — 기본 1920x1080 캔버스에서 1:1 픽셀 매핑·필러박스)로 교체해 MAD ≤ 2.0 계약을
> 유지한 채 **핵심 파리티 18/18 시나리오 재검증 PASS**. 320x240 픽스처의 재표본 분산(4~36)은
> 픽스처/캔버스 스케일 불일치였고 허용치는 변경하지 않았다. 두 번째 클립 픽스처는 코덱 잡음
> (Perceptual 영역)이 Tolerance 게이트를 오염하지 않도록 평탄 smptebars로 생성.
...
| 16 | `curves_only` | G-02 Inc6 톤 커브 체인(마스터 S-커브+레드 리프트, 3-way·밴드 없음)의 프리뷰↔출력 동일성 — 커브 편집 UI가 커밋하는 값의 실증(밴드 체인과 독립 분리, 2026-08-26 신설 · 스크립트 16b) | ≤ 2.0 |
| 17 | `karaoke_text` | G-01 Inc2 카라오케 활성 단어 하이라이트의 프리뷰↔출력 동일성(단어 시작 전/후 하이라이트 상태 각각 샘플) | ≤ 2.0 |
| 18 | `motion_tracking` | 모션 트래킹 적용 구간의 프리뷰↔출력 동일성(2026-08-22 문서 정합성 검토에서 표에 누락된 것을 보강 — 스크립트 314행 활성 시나리오) | ≤ 2.0 |

> 번호 주석: 표의 #는 위 표 기준. `run_core_editing_parity.sh` 내 시나리오 번호는 전환(transition) 시나리오 1번이 주석 처리되어 있어 +1 차이난다(#13 `crop_rect` = 스크립트 14번, #14 `crop_rect_video` = 스크립트 15번, #15 `hsl_curves` = 스크립트 16번, #16 `curves_only` = 스크립트 16b, #17 `karaoke_text` = 스크립트 17번).
