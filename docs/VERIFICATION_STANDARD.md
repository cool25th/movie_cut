...
> 번호 주석: 표의 #는 위 표 기준. `run_core_editing_parity.sh` 내 시나리오 번호는 전환(transition) 시나리오 1번이 주석 처리되어 있어 +1 차이난다(#13 `crop_rect` = 스크립트 14번, #14 `crop_rect_video` = 스크립트 15번, #15 `hsl_curves` = 스크립트 16번).
