...
> **파리티의 등급 정의(2026-08-22)** — "완전히 같은 픽셀"은 측정 불가능한 주장이므로 3등급으로 정의한다:
> ① **Exact**(시간 계산·키프레임 평가·transform·렌더 계획·무손실 중간 출력): 수치 동일. ② **Tolerance**(GPU 색보정·마스크 경계·필터): 허용 오차 — 아래 MAD ≤ 2.0이 이 등급의 현행 수치이며, 등급별 세분 수치 확정은 진행 중. ③ **Perceptual**(최종 코덱 인코딩·프록시 프리뷰·HDR 디스플레이): 지각적 일치(블라인드 비교). 외부 문구는 "보이는 대로 출력"을 사용한다.
