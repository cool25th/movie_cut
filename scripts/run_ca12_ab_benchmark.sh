...
for fixture in "${FIXTURES[@]}"; do
  ...
  # Aggregate ALL reps into the run summary (CODEX-11: the rep1-only read
  # recorded a single sample while the conditions note promised median/p95
  # per rep list). rep1 stays the representative record for backward
  # compatibility; per-field statistics across every rep live in
  # repetition_stats, and a single-rep run says so instead of dressing one
  # sample up as statistics.
  python3 - "$id" "$RUN_DIR/$id/rep1" "$RUN_DIR/$id" "$RUN_DIR" "$REPS" <<'PY'
import json, math, shutil, sys
from pathlib import Path
fid, rep1, fixture_dir, run, reps_total = (
    sys.argv[1], Path(sys.argv[2]), Path(sys.argv[3]), Path(sys.argv[4]), int(sys.argv[5]),
)

rep_dirs = sorted(
    (d for d in fixture_dir.glob("rep[0-9]*") if d.is_dir()),
...
for d in rep_dirs:
    t = d / "timing.json"
    if t.exists():
        timings.append(json.loads(t.read_text()))
...
def field_stats(values):
    ...
    if not values:
        return None
    stats = {"n": len(values), "median": round(values[len(values) // 2], 4)}
    if len(values) >= 2:
        # Nearest-rank p95 — honest at small n (n=2 → the max).
        rank = max(1, math.ceil(0.95 * len(values)))
        stats.update(
            p95=round(values[rank - 1], 4),
            min=round(values[0], 4),
            max=round(values[-1], 4),
        )
    else:
        stats.update(p95=None, single_rep=True)
    return stats

entry = {"fixture": fid, "reps_recorded": len(rep_dirs)}
if len(rep_dirs) != reps_total:
    entry["reps_expected"] = reps_total
failed = [d.name for d in rep_dirs if (d / "metrics_error.json").exists()]
if failed:
    entry["failed_reps"] = failed
if timings:
    entry["repetition_stats"] = {
        key: field_stats([t.get(key) for t in timings])
        for key in sorted({k for t in timings for k in t})
    }
...
print(f"summarized {fid} ({len(rep_dirs)} rep(s), stats fields: {len(entry.get('repetition_stats', {}))})")
