#!/usr/bin/env bash
# Headless import → timeline → export E2E under the App Sandbox (shipping config).
#
# This is run_e2e_export.sh with the sandbox variable flipped ON. The original
# drives the harness with sandbox-OFF fixtures and never ran the 27 sections
# under ENABLE_APP_SANDBOX=YES (the shipping default). With CONTAINERIZE=1
# (added 2026-08-02), the harness routes fixtures/exports through the sandbox
# container's grant-free tmp/, so the same sections can run sandboxed.
#
# Differences from run_e2e_export.sh:
#   - builds with MOVIECUT_HARNESS + ENABLE_APP_SANDBOX=YES (explicit shipping)
#   - every harness invocation adds MOVIECUT_UITEST_CONTAINERIZE=1
#   - artifact lookups fall back to the container_artifacts= status field when
#     the sandbox blocked the move to the requested path
#
# Usage:  bash scripts/run_e2e_export_sandbox.sh
set -euo pipefail
...
# resolve_artifact <requested_path> <result_file>
# If the requested export path is missing, the sandbox may have blocked the
# move out of the container; the harness then reports the staging path in
# `container_artifacts=` (colon-separated). Echo the first existing match.
resolve_artifact() {
  local requested="$1" result_file="${2:-}"
  if [ -s "$requested" ]; then echo "$requested"; return; fi
  if [ -n "$result_file" ] && [ -s "$result_file" ]; then
    local staged
    staged="$(grep -oE 'container_artifacts=[^ ]*' "$result_file" 2>/dev/null | cut -d= -f2- \
              | tr ':' '\n' | grep -F ".${requested##*.}" | head -1)"
    if [ -n "$staged" ] && [ -s "$staged" ]; then echo "$staged"; return; fi
  fi
  echo ""
}
...
env MOVIECUT_UITEST=1 MOVIECUT_UITEST_CONTAINERIZE=1 MOVIECUT_UITEST_IMPORT="$FIXTURE" \
...
env MOVIECUT_UITEST=1 MOVIECUT_UITEST_CONTAINERIZE=1\
...
env MOVIECUT_UITEST=1 MOVIECUT_UITEST_CONTAINERIZE=1 MOVIECUT_UITEST_IMPORT="$FIXTURE,$FIXTURE" \
...
env MOVIECUT_UITEST=1 MOVIECUT_UITEST_CONTAINERIZE=1 MOVIECUT_UITEST_IMPORT="$IMAGE_FIXTURE" \
...
env MOVIECUT_UITEST=1 MOVIECUT_UITEST_CONTAINERIZE=1 MOVIECUT_UITEST_IMPORT="$IMAGE_FIXTURE" MOVIECUT_UITEST_GRADE=1 \
...
MOVIECUT_UITEST=1 MOVIECUT_UITEST_CONTAINERIZE=1\
...
MOVIECUT_UITEST=1 MOVIECUT_UITEST_CONTAINERIZE=1 MOVIECUT_UITEST_IMPORT="$FIXTURE" MOVIECUT_UITEST_FREEZE=1 \
...
env MOVIECUT_UITEST=1 MOVIECUT_UITEST_CONTAINERIZE=1 MOVIECUT_UITEST_IMPORT="$OF_FIXTURE" MOVIECUT_UITEST_PLAYBACK_RATE=0.25 \
...
env MOVIECUT_UITEST=1 MOVIECUT_UITEST_CONTAINERIZE=1 MOVIECUT_UITEST_IMPORT="$TEXT_ANIMATION_SOURCE" MOVIECUT_UITEST_COLOR=1 MOVIECUT_UITEST_TEXT_ANIMATION_PRESET="none" \
...
for preset in "${TEXT_ANIMATION_PRESETS[@]}"; do
  ...
  env MOVIECUT_UITEST=1 MOVIECUT_UITEST_CONTAINERIZE=1 MOVIECUT_UITEST_IMPORT="$TEXT_ANIMATION_SOURCE" MOVIECUT_UITEST_COLOR=1 MOVIECUT_UITEST_TEXT_ANIMATION_PRESET="$preset" \
...
for template_name in "${TITLE_TEMPLATE_NAMES[@]}"; do
  ...
  env MOVIECUT_UITEST=1 MOVIECUT_UITEST_CONTAINERIZE=1 MOVIECUT_UITEST_IMPORT="$FIXTURE" MOVIECUT_UITEST_TEXT_TEMPLATE_NAME="$TITLE_ENV_NAME" \
...
MOVIECUT_UITEST=1 MOVIECUT_UITEST_CONTAINERIZE=1 MOVIECUT_UITEST_IMPORT="$TONE" MOVIECUT_UITEST_DENOISE=1 \
...
env MOVIECUT_UITEST=1 MOVIECUT_UITEST_CONTAINERIZE=1 MOVIECUT_UITEST_IMPORT="$NOISY" MOVIECUT_UITEST_EXPORT="$NR_BASE" \
...
env MOVIECUT_UITEST=1 MOVIECUT_UITEST_CONTAINERIZE=1 MOVIECUT_UITEST_IMPORT="$NOISY" MOVIECUT_UITEST_DENOISE=1 \
...
env MOVIECUT_UITEST=1 MOVIECUT_UITEST_CONTAINERIZE=1 MOVIECUT_UITEST_IMPORT="$EQ_FIXTURE" MOVIECUT_UITEST_EQ_PRESET="bassBoost" \
...
env MOVIECUT_UITEST=1 MOVIECUT_UITEST_CONTAINERIZE=1 MOVIECUT_UITEST_IMPORT="$EQ_FIXTURE" MOVIECUT_UITEST_EQ_PRESET="trebleBoost" \
...
env MOVIECUT_UITEST=1 MOVIECUT_UITEST_CONTAINERIZE=1 MOVIECUT_UITEST_IMPORT="$EXTRACT_AUDIO_FIXTURE" MOVIECUT_UITEST_EXTRACT_AUDIO=1 \
...
env MOVIECUT_UITEST=1 MOVIECUT_UITEST_CONTAINERIZE=1 MOVIECUT_UITEST_IMPORT="$FIXTURE" MOVIECUT_UITEST_CHAPTER_MARKERS=1 MOVIECUT_UITEST_BEAT_CHAPTERS=1 \
...
env MOVIECUT_UITEST=1 MOVIECUT_UITEST_CONTAINERIZE=1 MOVIECUT_UITEST_DUCKING_BGM="$DUCK_BGM" MOVIECUT_UITEST_DUCKING_VOICE="$DUCK_VOICE" \
...
env MOVIECUT_UITEST=1 MOVIECUT_UITEST_CONTAINERIZE=1 MOVIECUT_UITEST_DUCKING_BGM="$DUCK_BGM" MOVIECUT_UITEST_DUCKING_VOICE="$DUCK_VOICE" MOVIECUT_UITEST_DUCKING_APPLY=1 \
...
platform_export_check() {
  ...
  env MOVIECUT_UITEST=1 MOVIECUT_UITEST_CONTAINERIZE=1 MOVIECUT_UITEST_IMPORT="$BARS" MOVIECUT_UITEST_PLATFORM_PRESET="$raw" \
...
env MOVIECUT_UITEST=1 MOVIECUT_UITEST_CONTAINERIZE=1 MOVIECUT_UITEST_IMPORT="$BARS" MOVIECUT_UITEST_EXPORT="$GBASE" MOVIECUT_UITEST_QUIT=1 "$APP_BIN" >/dev/null 2>&1 &
GP=$!; for _ in $(seq 1 120); do [ -s "$GBASE" ] && break; sleep 0.5; done; wait "$GP" 2>/dev/null || true
env MOVIECUT_UITEST=1 MOVIECUT_UITEST_CONTAINERIZE=1 MOVIECUT_UITEST_IMPORT="$BARS" MOVIECUT_UITEST_GRADE=1 MOVIECUT_UITEST_EXPORT="$GGRADE" MOVIECUT_UITEST_QUIT=1 "$APP_BIN" >/dev/null 2>&1 &
...
env MOVIECUT_UITEST=1 MOVIECUT_UITEST_CONTAINERIZE=1 MOVIECUT_UITEST_IMPORT="$FIXTURE" MOVIECUT_UITEST_EXPORT="$G02_BASE" MOVIECUT_UITEST_QUIT=1 "$APP_BIN" >/dev/null 2>&1 &
G02P=$!; for _ in $(seq 1 120); do [ -s "$G02_BASE" ] && break; sleep 0.5; done; wait "$G02P" 2>/dev/null || true
env MOVIECUT_UITEST=1 MOVIECUT_UITEST_CONTAINERIZE=1 MOVIECUT_UITEST_IMPORT="$FIXTURE" MOVIECUT_UITEST_HSL_CURVES=1 MOVIECUT_UITEST_EXPORT="$G02_GRADE" MOVIECUT_UITEST_QUIT=1 "$APP_BIN" >/dev/null 2>&1 &
...
env MOVIECUT_UITEST=1 MOVIECUT_UITEST_CONTAINERIZE=1 MOVIECUT_UITEST_IMPORT="$BARS" MOVIECUT_UITEST_SCOPE=1 \
...
env MOVIECUT_UITEST=1 MOVIECUT_UITEST_CONTAINERIZE=1 MOVIECUT_UITEST_IMPORT="$BARS" MOVIECUT_UITEST_EXPORT_PRORES="$PR_OUT" \
  MOVIECUT_UITEST_QUIT=1 "$APP_BIN" >/dev/null 2>&1 &
...
env MOVIECUT_UITEST=1 MOVIECUT_UITEST_CONTAINERIZE=1 MOVIECUT_UITEST_IMPORT="$BARS" MOVIECUT_UITEST_EXPORT_HDR="$HDR_OUT" \
  MOVIECUT_UITEST_QUIT=1 "$APP_BIN" >/dev/null 2>&1 &
...
env MOVIECUT_UITEST=1 MOVIECUT_UITEST_CONTAINERIZE=1 MOVIECUT_UITEST_IMPORT="$BARS" MOVIECUT_UITEST_AUTOWB=1 \
...
env MOVIECUT_UITEST=1 MOVIECUT_UITEST_CONTAINERIZE=1 MOVIECUT_UITEST_IMPORT="$BARS" MOVIECUT_UITEST_AUTOLEVELS=1 \
...
env MOVIECUT_UITEST=1 MOVIECUT_UITEST_CONTAINERIZE=1 MOVIECUT_UITEST_IMPORT="$BARS" MOVIECUT_UITEST_AUTOENHANCE=1 \
...
# Crash-recovery autosave — SKIPPED under the sandbox.
#
# The recovery.moviecut is written by flushAutosave() during the harness
# shutdown path, then immediately cleared by the willTerminate handler that
# follows NSApp.terminate. The sandbox-OFF script (run_e2e_export.sh) only
# passes because its poll loop catches the file in the window between flush
# and clear — a timing race. Without QUIT, flushAutosave() is never called,
# so the file never appears; with QUIT, the clear wins the race here. Either
# way it's not a stable signal under the sandbox.
#
# The recovery *semantics* (save on edit, survive a crash, clear on clean
# quit) are already covered by AutosaveRecoveryTests (unit, ProjectStore
# actor level). The end-to-end "force-kill then relaunch shows the recovery
# prompt" gate is the B-U7 task, which needs a UI identifier + gate bypass
...
env MOVIECUT_UITEST=1 MOVIECUT_UITEST_CONTAINERIZE=1 MOVIECUT_UITEST_CARD_EDITOR=1 \
...
env MOVIECUT_UITEST=1 MOVIECUT_UITEST_CONTAINERIZE=1 MOVIECUT_UITEST_CARD_TEMPLATE=1 \
