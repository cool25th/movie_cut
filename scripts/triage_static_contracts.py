...
def main() -> int:
    ...
    lines.append("")
    lines.append("> **Layout/polish coverage:** a dhash screenshot regression gate ")
    lines.append("> (`scripts/ui_regression.sh`, nightly) compares committed goldens ")
    lines.append("> under `Tests/UIEvidence/`. It catches whole-panel ")
    lines.append("> appearance/disappearance but is text/subtle-change blind, so it ")
    lines.append("> COMPLEMENTS (does not replace) the layout checks listed under ")
    lines.append("> REPLACE below. Wiring/copy checks remain XCUITest territory. ")
    lines.append("")
